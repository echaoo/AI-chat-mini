
  设计方案：主对话 + 副本/小剧场

  概念定义

  主对话（Main Conversation）
  - 🏠 日常陪伴，连续性对话
  - 💭 AI 记住所有互动历史
  - ♾️ 无限延续，永不结束
  - 📍 固定到首页，一键进入

  副本/小剧场（Dungeon/Scene）
  - 🎭 特定场景的独立对话
  - 🎬 有剧情、有目标、有结局
  - ⏱️ 有限时长，完成后关闭
  - 🎁 可解锁、可收费、可分享

  ---
  产品形态设计

  1. 用户界面结构

  ┌─────────────────────────┐
  │       首 页            │
  │  ┌─────────────────┐   │
  │  │  祁煜 (主对话)   │   │
  │  │  "早安，想我了吗" │   │
  │  │  [💬 聊天]       │   │
  │  └─────────────────┘   │
  │                         │
  │  🎭 小剧场入口          │
  │  ┌───┬───┬───┐         │
  │  │雨天│约会│生日│  ...   │
  │  └───┴───┴───┘         │
  └─────────────────────────┘

  2. 小剧场场景示例

  免费场景：
  - 🌧️ 雨天漫步 - "突然下雨了，祁煜给你递来雨伞..."
  - ☕ 午后咖啡馆 - "阳光洒在咖啡杯上，他望向你..."
  - 🌙 深夜电话 - "夜深了，手机响起，是他的来电..."

  付费场景：
  - 💝 生日惊喜 - 完整的生日剧情，AI记住你的生日
  - 🎆 烟花大会 - 夏日祭典，浴衣约会
  - 🏖️ 海边度假 - 3天2夜的度假剧情

  限时场景：
  - 🎄 圣诞特辑 - 仅12月开放
  - 🎊 新年倒数 - 跨年专属剧情
  - 💕 情人节 - 214限定

  ---
  技术实现方案

  数据结构设计

  // 对话类型
  enum ConversationType {
    PRIMARY = 'primary',      // 主对话
    DUNGEON = 'dungeon',      // 副本
    SCENE = 'scene'           // 小剧场
  }

  // 对话表
  interface Conversation {
    id: number
    userId: number
    characterId: number
    type: ConversationType

    // 副本相关字段
    dungeonId?: number        // 副本模板ID
    sceneTitle?: string       // 场景标题
    sceneDescription?: string // 场景描述
    status: 'active' | 'completed' | 'expired'
    expiresAt?: string        // 过期时间

    // 通用字段
    messageCount: number
    createdAt: string
    updatedAt: string
  }

  // 副本模板表（官方设计的剧情）
  interface DungeonTemplate {
    id: number
    characterId: number
    title: string             // "雨天漫步"
    description: string       // 场景描述
    coverImage: string        // 封面图

    // 剧情设定
    systemPrompt: string      // 给AI的剧情背景提示
    initialMessage: string    // 开场白
    estimatedMessages: number // 预计对话轮数

    // 解锁条件
    unlockType: 'free' | 'paid' | 'level'
    unlockCost?: number       // 积分消耗
    unlockLevel?: number      // 需要等级

    // 时间限制
    availableFrom?: string    // 限时开放开始
    availableTo?: string      // 限时开放结束
    duration?: number         // 副本时长(分钟)

    tags: string[]            // ['浪漫', '治愈', '剧情向']
    isRecommended: boolean    // 是否推荐
    playCount: number         // 游玩次数
    createdAt: string
  }

  API 设计

  // 1. 获取角色的主对话
  GET /companion/characters/:characterId/primary-conversation
  Response: { conversation, messages }

  // 2. 获取可用的副本列表
  GET /companion/characters/:characterId/dungeons
  Response: {
    free: DungeonTemplate[],
    paid: DungeonTemplate[],
    limited: DungeonTemplate[],
    locked: DungeonTemplate[]
  }

  // 3. 开始副本（创建副本对话）
  POST /companion/dungeons/:dungeonId/start
  Request: { characterId }
  Response: {
    conversation,      // 新创建的副本对话
    initialMessage,    // AI开场白
    expiresAt         // 过期时间
  }

  // 4. 获取副本历史
  GET /companion/dungeons/history
  Response: {
    active: Conversation[],     // 进行中的副本
    completed: Conversation[]   // 已完成的副本
  }

  ---
  前端实现示例

  1. 角色详情页（选择主对话或副本）

  <template>
    <view class="character-page">
      <!-- 主对话卡片 -->
      <view class="main-conversation" bindtap="enterMainChat">
        <text class="title">与{{character.name}}的日常</text>
        <text class="last-message">{{lastMessage}}</text>
        <button>继续聊天</button>
      </view>

      <!-- 副本列表 -->
      <view class="dungeons-section">
        <text class="section-title">🎭 小剧场</text>

        <scroll-view scroll-x class="dungeon-list">
          <view
            wx:for="{{dungeons}}"
            wx:key="id"
            class="dungeon-card {{item.locked ? 'locked' : ''}}"
            bindtap="startDungeon"
            data-id="{{item.id}}"
          >
            <image src="{{item.coverImage}}" class="cover"/>
            <text class="title">{{item.title}}</text>
            <text class="desc">{{item.description}}</text>

            <view wx:if="{{item.unlockType === 'paid'}}" class="cost">
              💎 {{item.unlockCost}}
            </view>
            <view wx:if="{{item.limited}}" class="limited">
              ⏰ 限时
            </view>
          </view>
        </scroll-view>
      </view>
    </view>
  </template>

  2. 聊天页面（区分主对话和副本）

  createPage({
    data: {
      conversationType: 'primary' as ConversationType,
      dungeonInfo: null as DungeonTemplate | null,
      expiresAt: null as string | null
    },

    onLoad(options: any) {
      if (options.dungeonId) {
        // 副本模式
        this.conversationType = 'dungeon'
        this.startDungeon(options.dungeonId)
      } else {
        // 主对话模式
        this.conversationType = 'primary'
        this.loadPrimaryConversation(options.characterId)
      }
    },

    async startDungeon(dungeonId: number) {
      // 消耗积分，创建副本对话
      const result = await dungeonApi.start(dungeonId, this.characterId)

      this.conversationId = result.conversation.id
      this.dungeonInfo = result.dungeonInfo
      this.expiresAt = result.expiresAt

      // 显示AI开场白
      this.messages = [{
        role: 'assistant',
        content: result.initialMessage
      }]
    }
  })

  ---
  UI/UX 细节

  副本入口设计

  方案 1：首页入口
  首页
    ├─ 主角色卡片（固定角色）
    ├─ [💬 聊天] 按钮 → 进入主对话
    └─ 🎭 小剧场轮播图 → 点击进入副本列表

  方案 2：角色详情页
  角色列表
    ↓ 点击角色
  角色详情页
    ├─ 角色介绍
    ├─ [进入主对话] 按钮
    └─ 小剧场列表（横向滚动）

  方案 3：聊天页内入口
  聊天页面
    ├─ 右上角 [...] 菜单
    └─ [🎭 进入小剧场] 选项

  ---
  商业价值

  变现点

  1. 副本解锁
    - 付费副本：高质量剧情，专业编剧
    - 会员专属：VIP 用户独享场景
    - 限时活动：节日限定，饥饿营销
  2. 积分消耗
    - 普通副本：消耗积分进入
    - 重复游玩：想重温剧情需要再次付费
    - 解锁结局：多结局副本，解锁不同结局
  3. UGC 内容
    - 用户创作：用户自己编写副本
    - 分享收益：优质副本上架，分成收益

  用户留存

  - ✅ 每日/每周更新新副本 → 提高访问频率
  - ✅ 限时副本 → FOMO 心理，促进活跃
  - ✅ 剧情连续性 → 追剧式体验，提高粘性
  - ✅ 收集成就 → 完成所有副本解锁特殊奖励

  ---
  实现优先级建议

  MVP（最小可行产品）

  Phase 1：基础功能
  1. ✅ 主对话功能（一个角色一个对话）
  2. ✅ 固定到首页
  3. ⭕ 副本模板数据结构
  4. ⭕ 1-2个免费副本体验

  Phase 2：内容扩展
  5. ⭕ 副本列表页面
  6. ⭕ 5-10个免费副本
  7. ⭕ 积分系统 + 付费副本
  8. ⭕ 副本历史记录

  Phase 3：高级功能
  9. ⭕ 限时副本
  10. ⭕ 多结局系统
  11. ⭕ 用户创作副本
  12. ⭕ 副本分享/推荐

  ---
  我的建议

  你的"副本/小剧场"思路非常棒！建议：

  1. 先做好主对话 - 保证核心体验流畅
  2. 设计2-3个高质量副本 - 验证用户喜好
  3. 观察数据 - 看用户更喜欢主对话还是副本
  4. 迭代优化 - 根据反馈调整副本设计
 No newline at end of file
